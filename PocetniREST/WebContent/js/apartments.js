$(document).ready(function () {
    ...
    if (searchParams.has('id')) {
        ...
        $.get({
            url: '/PocetniREST/rest/apartments/apartmentsActive',
...
function loadApartments(apartments) {
    ...
    // rezervacija
    apartments.forEach(apartment => {
        $('#reserve' + apartment.id).click(function(){
            window.location.href = "http://localhost:8080/PocetniREST/html/reservation.html?id=" + apartment.id;
        });
    });
                            ...
                            '<div class="row">' +
                                '<div class="col text-right">' +
                                    '<div class="d-flex flex-column">' +
                                        '<button id="reserve' + apartments[i].id + '" type="button" class="btn btn-outline-warning">Reserve</button>' +
                                    '</div>' +
                                '</div>'+
                            '</div>' +
                            '<div class="row mt-2">' +
